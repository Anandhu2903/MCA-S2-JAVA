import java.io.*;
import java.util.*;

public class EmployeeFileIO {
    public static void main(String[] args) {
        try (Scanner obj = new Scanner(System.in);
             FileOutputStream fout = new FileOutputStream("employee2.txt")) {

            int empno;
            String empname;
            int salary;

            System.out.println("Enter the number of employees:");
            int limit = obj.nextInt();
            obj.nextLine(); // Consume the newline

            for (int j = 0; j < limit; j++) {
                try {
                    System.out.println("Enter the employee number:");
                    empno = obj.nextInt();
                    obj.nextLine(); // Consume the newline

                    System.out.println("Enter the employee name:");
                    empname = obj.nextLine();

                    System.out.println("Enter the employee salary:");
                    salary = obj.nextInt();
                    obj.nextLine(); // Consume the newline

                    String employeeData = empno + System.lineSeparator()
                                        + empname + System.lineSeparator()
                                        + salary + System.lineSeparator();

                    fout.write(employeeData.getBytes());

                } catch (InputMismatchException e) {
                    System.out.println("Invalid input. Please enter valid data types.");
                    obj.nextLine(); // clear invalid input
                    j--; // retry current employee entry
                }
            }

        } catch (IOException e) {
            System.out.println("Error writing to file: " + e.getMessage());
        }

        // Reading the file
        try (FileInputStream fin = new FileInputStream("employee2.txt")) {
            System.out.println("\nContents of the file:");
            int data;
            while ((data = fin.read()) != -1) {
                System.out.print((char) data);
            }
        } catch (IOException e) {
            System.out.println("Error reading file: " + e.getMessage());
        }
    }
}
